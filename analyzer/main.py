...
async def get_notes(filename: str):
    """Return notes for a recording (stored as {base}.notes.txt)."""
    base = Path(filename).stem
    notes_path = RECORDINGS_PATH / f"{base}.notes.txt"
...
async def save_notes(filename: str, payload: dict = Body(...)):
...
async def rename_file(payload: dict = Body(...)):
    """Rename an audio file and all associated files (.analysis.json and .notes.txt)."""
    old_name = payload.get("old_name")
    new_base = payload.get("new_name")  # User provides base name, we keep original extension

    if not old_name or not new_base:
        raise HTTPException(status_code=400, detail="old_name and new_name are required")

    old_audio = RECORDINGS_PATH / old_name
    if not old_audio.exists():
        raise HTTPException(status_code=404, detail="Original file not found")

    original_suffix = old_audio.suffix.lower()

    # Sanitize the base name provided by user
    clean_base = "".join(c for c in new_base if c.isalnum() or c in "._- ").strip()
    if not clean_base:
        raise HTTPException(status_code=400, detail="Invalid new filename")

    # Always keep the original extension
    new_name = clean_base
    if not new_name.lower().endswith(original_suffix):
        new_name = new_name + original_suffix

    new_audio = RECORDINGS_PATH / new_name
    ...
    try:
        ...
        # Rename analysis file if it exists
        old_analysis = old_audio.with_suffix(old_audio.suffix + ".analysis.json")
        if old_analysis.exists():
            new_analysis = new_audio.with_suffix(new_audio.suffix + ".analysis.json")
            old_analysis.rename(new_analysis)

        # Rename notes file if it exists
        old_notes = RECORDINGS_PATH / f"{old_audio.stem}.notes.txt"
        if old_notes.exists():
            new_notes = RECORDINGS_PATH / f"{new_audio.stem}.notes.txt"
